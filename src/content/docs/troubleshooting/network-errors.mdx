...
| Connection Timeout              | <Yes />        | Server accepted connection but never replied  |
...
| Connection Refused / Failure    | <No />         | Connection refused, DNS failure, blocked port |
...
connection in the first place &mdash; for example a refused port, a firewall
blocking the connection, or a DNS lookup that returned no result &mdash; so a
retry won't help. lychee reports these as `Connection refused - server may be
down or port blocked` or `Connection failed. Check network connectivity and
firewall settings`.
