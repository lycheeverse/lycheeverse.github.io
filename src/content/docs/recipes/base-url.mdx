---
title: Testing Sites Not Served from Root with --base
description: Understanding how to use the --base parameter in lychee
---
...
The `--base` parameter in lychee lets you check links for sites that aren't
...
Most of the time, you don't! Skip `--base` if:
...
You need `--base` when your site lives in a subdirectory.
...
:::note[Good to know]
The `--base` parameter works similarly to other tools you might know:
...
If you've used any of these, lychee's `--base` follows the same concept.
:::

## How Does It Work?

When you set `--base`, lychee will:
...
```bash
# For a project site
lychee --base https://username.github.io/project/ "**/*.html"

# For local development
lychee --base http://localhost:4000/project/ "**/*.html"
```
...
lychee --base https://example.com/docs/
...
- `--base` is for URLs (like `https://example.com/docs/`)
...
For more details, see the [troubleshooting guide](/guides/troubleshooting).
